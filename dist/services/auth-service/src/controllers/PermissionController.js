...
class PermissionController {
    ...
    async getPermissionById(req, res) {
        try {
            const { id } = req.params;
            const permission = await permission_model_1.Permission.findById(id);
            if (!permission) {
    ...
    async createPermission(req, res) {
        try {
            ...
            const existingPermission = await permission_model_1.Permission.findOne({ resource, action });
    ...
    async updatePermission(req, res) {
        try {
            ...
            const updatedPermission = await permission_model_1.Permission.findByIdAndUpdate(id, { resource, action, description }, { new: true });
    ...
    async deletePermission(req, res) {
        try {
            ...
            await permission_model_1.Permission.findByIdAndDelete(id);
    ...
    async getPermissionsByResource(req, res) {
        try {
            ...
            const permissions = await permission_model_1.Permission.find({ resource });
            res.status(200).json(permissions);
        }
        catch (error) {
     ...
     */
    async createMultiplePermissions(req, res) {
        try {
            ...
            if (existingPermissions.length > 0) {
                const existing = existingPermissions.map(p => `${p.resource}:${p.action}`).join(', ');
                res.status(400).json({
            ...
            const createdPermissions = await permission_model_1.Permission.insertMany(permissions);
