...
export const initializeOrderService = (app: Application, io: SocketIOServer): void => {
  ...
  // Initialize WebSocketService with the main application's Socket.IO instance
  try {
    WebSocketService.setServer(io);
    console.log('WebSocketService initialized with main Socket.IO instance');
  } catch (error) {
    console.error('Error initializing WebSocketService:', error);
  }

  // Initialize order socket service
  // We don't need this anymore as WebSocketService is now initialized with the main Socket.IO instance
