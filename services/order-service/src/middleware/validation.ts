...
export const validateUpdateOrder = (req: Request, res: Response, next: NextFunction): void => {
  try {
    ...
    if (req.body.status && !['PENDING', 'ACCEPTED', 'PREPARING', 'READY', 'DELIVERED', 'COMPLETED', 'CANCELLED', 'REJECTED'].includes(req.body.status.toUpperCase())) {
      res.status(HTTP_STATUS.BAD_REQUEST).json({
        status: 'error',
    ...
    if (req.body.paymentStatus && !['PENDING', 'PAID', 'FAILED', 'REFUNDED', 'PARTIALLY_REFUNDED'].includes(req.body.paymentStatus.toUpperCase())) {
      res.status(HTTP_STATUS.BAD_REQUEST).json({
        status: 'error',
