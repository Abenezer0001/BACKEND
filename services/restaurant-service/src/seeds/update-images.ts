...
import { McpClient } from '@modelcontextprotocol/sdk/client/index.js'; // Assuming SDK is installed or linked
import { StdioClientTransport } from '@modelcontextprotocol/sdk/client/stdio.js'; // Assuming SDK is installed or linked
...
const updateModelImages = async (Model: mongoose.Model<any>, modelName: string) => {
  ...
  const documents = await Model.find({ imageSearchTerm: { $exists: true, $ne: null, $ne: '' } }).exec();
